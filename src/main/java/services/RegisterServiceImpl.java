...
public class RegisterServiceImpl implements RegisterService {
    ...
    private void createResponse(RegisterRequest request) {
        ...
        if(usernameFree)
            response = new RegisterResponse(usernameFree, "Rejestracja zakończona powodzeniem");
        else
            response = new RegisterResponse(usernameFree, String.format("Użytkownik '%s' już istnieje", request.getUsername()));
    }
}
